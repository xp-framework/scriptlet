...
class HttpScriptletTest extends ScriptletTestCase {
  ...
  protected function assertHandlerForMethodTriggered($method) {
    $req= $this->newRequest($method, new \peer\URL('http://localhost/'));
  ...
  public function getSupported() {
    ...
    $s= new \scriptlet\HttpScriptlet($req, $res);
    $s->service($req, $res);
  }
  ...
  public function headSupported() {
    $req= $this->newRequest('HEAD', new \peer\URL('http://localhost/'));
    $res= new \scriptlet\HttpScriptletResponse();
  ...
  public function illegalHttpVerb() {
    $req= $this->newRequest('LOOK', new \peer\URL('http://localhost/'));
    $res= new \scriptlet\HttpScriptletResponse();
  ...
  public function requestedMethodNotImplemented() {
    $req= $this->newRequest('DELETE', new \peer\URL('http://localhost/'));
    $res= new \scriptlet\HttpScriptletResponse();
  ...
  public function invalidSessionCreatesNewSession() {
    $req= $this->newRequest('GET', new \peer\URL('http://localhost/?psessionid=INVALID'));
    $res= new \scriptlet\HttpScriptletResponse();
    ...
    $s= newinstance('scriptlet.HttpScriptlet', array(), '{
      ...
      public function handleSessionInitialization($request) {
        if (!preg_match("/^a-f0-9$/", $request->getSessionId())) { 
          throw new \lang\IllegalArgumentException("Invalid characters in session id");
        }
  ...
  public function unconditionalDenyAuthenticator() {
    $req= $this->newRequest('GET', new \peer\URL('http://localhost/members/profile'));
    $res= new \scriptlet\HttpScriptletResponse();
      ...
      public function getAuthenticator($request) {
        ...
        return newinstance(\'scriptlet.RequestAuthenticator\', array(), \'{
          public function authenticate($request, $response, $context) {
            throw new \lang\IllegalAccessException("Valid user required");
          }
        }\');
      }
    }');
