...
class XMLScriptletResponse extends HttpScriptletResponse {
  ...
  public function addFormValue($name, $values) {
    ...
    foreach ($values as $k => $val) {
      try {
        if (is_array($val)) {
          $c= \xml\Node::fromArray($val, 'param');
        } else if (is_object($val)) {
          $c= \xml\Node::fromObject($val, 'param');
        } else {
          $c= new \xml\Node('param', $val);
        }
  ...
  public function addFormResult($node) {
    if (
      ...
      throw new \lang\IllegalArgumentException($node->getName().' not allowed here');
    }
  ...
  public function process() {
    ...
    switch ($this->_stylesheet[0]) {
      case self::XSLT_FILE:
        try {
          ...
          throw new \scriptlet\ScriptletException($e->getMessage(), HttpConstants::STATUS_NOT_FOUND);
        }
        break;
    ...
    try {
      ...
      throw new \scriptlet\ScriptletException($e->getMessage(), HttpConstants::STATUS_INTERNAL_SERVER_ERROR);
    }
